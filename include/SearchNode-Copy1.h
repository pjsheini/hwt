...
SearchNode::SearchNode(UINT8* _code,UINT32 _B_over_8, int _max_lvl, int _K, Node* _root, int _NQ) {
    ...
    //}
    //cout<< " " <<endl;
    //UINT8 query_normchunks[0] = (UINT8)qn;
    //cout << num_chunks  << " : " << max_lvl<< " : "<<(int)query_normchunks[0] << "byte-> "<<sizeof(query_normchunks)<<endl;
...
void SearchNode::HNN_search() {
	...
	entry* current_entry;
    //std::unordered_map<UINT64,Node*> got;
	sparse_hash_map<UINT64,Node*>::iterator got;

	bool all_chilld = true;
    //cout<<  query_normchunks[0]-1 <<" herrrrre"<< endl;
	for(int i=0;i<=B;i++) {
        //cout<< i<<" ";
		...
		for(int stack_iter = hamm_radius%2 ;stack_iter <=hamm_radius;stack_iter = stack_iter + 2) {
			...
			while(cur!=nullptr) {
				if(false)
					...
					all_chilld = false;
                //printf("Hamming radius = %d  %ld %d\n",hamm_radius, sizeof(stacks_hammd)/sizeof(Node*), stack_iter);
...
void SearchNode::node_search(entry* ent, bool all_child,int stack_index) {
    //cout<<"herecheck1  "<< ent->current<< "iter "<< stack_index<<endl;
...
int SearchNode::get_size(int r, int c) {
    //cout<< r << " is r and c is "<< c<< "and max_c is "<< max_c<<endl;
